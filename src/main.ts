...
const svc = await createCoreService({ role: 'api' })
const { kernel, app } = svc
if (!app) throw new Error('API role produced no HTTP server')

await app.listen({ port: kernel.env.PORT, host: kernel.env.HOST })
kernel.log.info({ port: kernel.env.PORT }, 'core API listening')
